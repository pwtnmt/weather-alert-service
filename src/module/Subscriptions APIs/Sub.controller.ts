...
export class SubscriptionsController {
  ...
  async findAll(@Req() req: any): Promise<SubscriptionEntity[]> {
    console.log('--- SubscriptionsController.findAll ---'); // Debug Log
    console.log('req.user from JWT Guard:', req.user); // Debug Log
    const userId = req.user.id;
    console.log('Extracted userId:', userId);
    if (!userId) {
  ...
  async remove(
    ...
    @Param('id', ParseIntPipe) subscriptionId: number,
        ...
        console.log('--- SubscriptionsController.remove ---'); // Debug Log
        console.log('req.user from JWT Guard:', req.user);
        const userId = req.user.id;
        console.log('Extracted userId:', userId);
        if (!userId) {
